...
void CGEventSetIOHIDEvent(CGEventRef cgEvent, IOHIDEventRef iohidEvent) {
    ...
    /// Use SkyLight's setter on macOS 27. The setter copies the HID payload into the CGEvent, so an extra retain
    /// here would leak one HIDEvent for every simulated gesture event.
    if (@available(macOS 27.0, *)) {
        ...
        if (slEventSetIOHIDEvent) {
            ...
            /// The offsets below are known to be invalid on macOS 27. Do not write through an unknown pointer if
            /// Apple removes or renames the private setter.
            DDLogError("CGEventSetIOHIDEvent: Couldn't resolve SLEventSetIOHIDEvent on macOS 27.");
        }
        return;
    }
